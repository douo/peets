...
def test_metadata_original_release_date_only(hijack):
    with hijack() as mock:
        mock("movie_original_release_date_only.json")
...
def test_tvshow(hijack):
    with hijack() as mock:
        mock("tvshow.json", "tmdbsimple.TV._GET")
        mock("season.json", "tmdbsimple.TV_Seasons._GET")
        tmdb = TmdbMovieMetadata(language=Language.ZH, country=Country.CN)
